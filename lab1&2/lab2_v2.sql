...
/*
############ lab update ##################################
#add foreign key from 'jbmanager' to 'jbdept'
*/

alter table jbdept add foreign key (manager) references jbmanager(manager_id);

/* TEST
# show foreign key of 'jbmanager'
SELECT 
  TABLE_NAME,COLUMN_NAME,CONSTRAINT_NAME, REFERENCED_TABLE_NAME,REFERENCED_COLUMN_NAME
FROM
  INFORMATION_SCHEMA.KEY_COLUMN_USAGE
WHERE
  REFERENCED_TABLE_SCHEMA = 'jb' AND
  REFERENCED_TABLE_NAME = 'jbmanager';

+------------+-------------+-----------------+-----------------------+------------------------+
| TABLE_NAME | COLUMN_NAME | CONSTRAINT_NAME | REFERENCED_TABLE_NAME | REFERENCED_COLUMN_NAME |
+------------+-------------+-----------------+-----------------------+------------------------+
| jbdept     | manager     | jbdept_ibfk_1   | jbmanager             | manager_id             |
+------------+-------------+-----------------+-----------------------+------------------------+
